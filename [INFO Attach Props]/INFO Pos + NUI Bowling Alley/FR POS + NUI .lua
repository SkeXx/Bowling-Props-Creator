- La ressource créer est d'origine GLB
    },
    ...
    -- Coins de la boîte englobante locale du prop.
    boundingBoxCorners = {
    ...
    --  zone réellement utile pour la boule.
    lane = {
        ...
        -- Valeurs pratiques pour gameplay.
    ...
    player = {
        -- Position calculée depuis le centre de la lane et le bord arrière du modèle.
        start = vector3(-0.497068, 0.316544, -8.750000),
        aimAt = vector3(-0.497068, 0.316544, 8.607387),
        localForward = vector3(0.0, 0.0, 1.0),
    },

    -- Rigoles détectées : surface basse bleue + bandes de bord.
    gutters = {
        left = {
            floor = {
    ...
    -- Seuils simples pour détecter si la boule sort de la lane orange.
    ballDetection = {
        laneXMin = -1.655751,
        laneXMax =  0.661614,
        laneZMin = -8.537982,
    ...
    -- 1) surface basse rouge, 2) surface supérieure/rail rouge.
    ballReturn = {
        side = "right",
        ...
        -- Points locaux sur les surfaces mesurées, utiles pour placer/aligner des boules.
        lowerPoints = {
    ...
    -- Zone écran côté gauche. Mesurée sur le grand plan incliné.
    screen = {
        side = "left",

        -- Plan avant principal détecté sur les faces orientées vers l'intérieur.
        plane = {
        ...
        -- Zone complète du plan détecté, incluant cadre et marge.
        fullDetected = {
        ...
        -- Zone conseillée pour DUI/NUI : plus petite que le cadre pour éviter de dépasser.
        nui = {
    ...
    -- Zones globales utiles pour script.
    zones = {
        startBackArea = {
            xMin = -2.538000,
            xMax =  1.356000,
...
-- Conversion point local GLB -> monde FiveM.
-- heading = rotation Z FiveM du prop, en degrés.
-- Rappel : GLB Y devient FiveM Z.
